	...
	if(temp!=0x0f)
	{
		...
		if(temp!=0x0f)
		{
			...
			switch(temp)
			{
				case 0x0e: 
				{
					if(gongneng_flag==1) gongneng_flag=0;   //再次按下键返回没按下键的显示写法 
					shizhong_flag=1;moshi_flag=0; dianya_flag=0;pinlv_flag=0;
					
				}
				break;
				case 0x0d: if(gongneng_flag==1) {gongneng_flag=0;}
					dianya_flag=1;shizhong_flag=0;pinlv_flag=0;moshi_flag=0;
					break;
				case 0x0b: pinlv_flag=1;shizhong_flag=0;dianya_flag=0;TH0=TL0=0; break;
				case 0x07:
				if(pinlv_flag!=1){gongneng_flag=1;moshi_flag++;}
				else if(pinlv_flag==1){if(p_qiehuan==1) p_qiehuan=0; else {p_qiehuan=1;}}	
				else if(pinlv_flag==0){if(c_qiehuan==1) c_qiehuan=0;else {c_qiehuan=1;}}
				break;
			} 
			while(P3!=0x7f);
		}
