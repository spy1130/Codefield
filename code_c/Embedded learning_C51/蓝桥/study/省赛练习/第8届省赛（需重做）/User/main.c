...
//				}
				...
				if(Data_Flag[Seg_Index][ucRtc_Index] % 16 == 0x0a)//遇到16加1
					Data_Flag[Seg_Index][ucRtc_Index] += 6;
				if(Data_Flag[Seg_Index][ucRtc_Index] == (ucRtc_Index?0x60:0x24))//设置上下限
...
//				}
				Data_Flag[Seg_Index][ucRtc_Index]--;
				if(Data_Flag[Seg_Index][ucRtc_Index] % 16 == 0x0f)//16里面遇到f转换成BCD码
					Data_Flag[Seg_Index][ucRtc_Index] -= 6;
				if(Data_Flag[Seg_Index][ucRtc_Index] == 0xf9)//下限设置（0H-7H  ）
...
void Led_Proc()
{
	...
	if((ucRtc[2] % 16) == (Alarm[2] % 16 + 5))
		Beep_Flag = 0;
	ucLed[0] = Led_Star_Flag & Beep_Flag;//需要同时成立
}
