...
/**********************定义某些管脚********************/
bit candle_fire_1=0x00;//  1 is fire on ; 0 is extinguish
bit candle_fire_9=0x00;//  1 is fire on ; 0 is extinguish
bit candle_fire_3=0x01;////  1 is fire on ; 0 is extinguish
...
aaa:
			if(candle_fire_1==0x01)
			{
				...
				while(front_dis > 189 );/////   23.05.25   182cm   太近还需调整   185cm 还是有点近
				...
				{
					Fan_On;
					delay_ms(1500);  //原本是1500的   23 .05.26 改为500尝试一下
					break;
				}
				while(LeftFire==0||RightFire==0);
...
//								find_fire_turn_right();
				...
				//now position is corner 2 of room 1,and next action put off fire 2			
						...
						{
							Fan_On;
							delay_ms(500);         //  23.05.26  改为 500ms  原本1500
							break;
						}
						while(LeftFire==0||RightFire==0);
...
///////////////
							...
							{
								Fan_On;
								delay_ms(500);     //原本是1500   23 .05.26 改为500尝试
								break;
							}
							while(LeftFire==0||RightFire==0);
