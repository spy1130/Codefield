...
bit candle_fire_2=0x00;////  1 is fire on ; 0 is extinguish
...
#define Fan_On {Fan = 0;tingche();tingche();tingche();tingche();tingche();}
...
aaa:
			if(candle_fire_1==0x01)
			{
				...
				while(front_dis > 189 );/////   23.05.25   182cm   太近还需调整   185cm 还是有点近 23.10.5 原189
				...
				{
					Fan_On;
					delay_ms2(500);  //原本是1500的   23 .05.26 改为500尝试一下
					break;
				}
				while(LeftFire==0||RightFire==0);
...
////////////////////
				 ...
				 if(candle_fire_9==0x01)
				 {
				    ...
				    right_dis= Ultrasonic_Get_Right();//右侧检查
					  ...
					  //now position is middle gate of room 1,and next action turn left 
						...
						while(front_dis > 215);//改为20-5.19，23-5.20   230  250    230  210合适
						delay_ms(50);//-------------------
		...
		////////////////////						
						...
						while(front_dis > 220);//原数据185改160-5.18，19-5.20,21-5.25     210   215
...
//								find_fire_turn_right();
				...
				//now position is corner 2 of room 1,and next action put off fire 2			
						...
						{
							Fan_On;
							delay_ms2(600);         //  23.05.26  改为 500ms  原本1500
							break;
						}
						while(LeftFire==0||RightFire==0);
...
//								find_fire_turn_back_left();
		...
		////////////////////							
					...
					//----------------------------------	
						...
						do
						{
				//			front_dis= kalmanfilter(Ultrasonic_Get_Front(),1);//卡尔曼滤波     3.28删除
							Circulation_wall_left();	//寻左墙		
						}
						while(front_dis > 190);//18cm，230-5.20    3.23-260    180合适   175稍微有点紧    
...
////////////////////						
						...
						right_dis=kalmanfilter(Ultrasonic_Get_Right(),3);
						right_dis=kalmanfilter(Ultrasonic_Get_Right(),3);
						right_dis=kalmanfilter(Ultrasonic_Get_Right(),3);
						...
						do{Circulation_wall_right();Circulation_wall_right();}while(right_dis<300);
						i=0;
						do{forward();}while(i<40);
						car_stop();
						car_stop();
					

						turn_right_90_small();//右转90°
						car_stop();
...
///////////////
						...
						if(candle_fire_2==0x01)//蜡烛2燃烧
						{
							...
							}while(front_dis >195);//前距离20cm，190-5.20     190可以使用 稍微近一些  
						do
							{
								Circulation_wall_left();
							}while(front_dis >195);//前距离20cm，190-5.20     190可以使用 稍微近一些 
...
///////////////
							...
							{
								Fan_On;
								delay_ms2(500);     //原本是1500   23 .05.26 改为500尝试
								break;
							}
							while(LeftFire==0||RightFire==0);
	...
	//						car_stop();
                ...
                //-----------------防误判-----2回------	
					...
					//----------------------------------	
							...
							{
								Circulation_wall_right();
							}while(front_dis >198);//170,190-5.20     190可以使用 稍微近一些---------------改  3.24  195 还是有点近   198
						// now potion is cornre 3 of room 2,next action  turn left
							...
							if(candle_fire_3==0x01)
							{
								...
								{
									Circulation_wall_right();
								}while(front_dis >190);//之前是210,170-5.20
									car_stop();
								  car_stop();
	...
	//							find_fire_turn_back_right();
							...
							else //---------------无3--
							{
								...
								{
									Circulation_wall_right();
								}while(front_dis >188);  //180可以走 稍微远    改  170太近了  -------------------改  3.24  175  178有点近   180可但还需要调节  185之前
								car_stop();
				 ...
				 else //------------------无2---
				 {
						...
						car_stop();     //  4.06加  
					turn_left_90_big31();  //灭火之前转过的弯太小了
				//		turn_left_90();
						car_stop();     //
						do
						{
...
//						 forward();
							Circulation_wall_right();		 
						}while(front_dis >188);    ///     原200     ----4.04  188
						car_stop();
						turn_left_90();
						car_stop();
...
///////////////						
						...
						{
							Circulation_wall_left();
						}while(front_dis >210);   //200可以使用   可以调大一些  ------------改210  3.24
						car_stop();
						turn_right_90();
/////////////						
