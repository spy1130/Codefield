...
{
    GPIO_InitTypeDef gpio_init_struct;
    FSMC_NORSRAM_TimingTypeDef fmc_read_handle;
    FSMC_NORSRAM_TimingTypeDef fmc_write_handle;
    ...
    /*FMC读时序控制寄存器*/
    fmc_read_handle.AddressSetupTime = 0x15;            /* 地址建立时间(ADDSET)为15个fmc_ker_ck 1/220M=4.5ns*15=67.5ns */
    fmc_read_handle.AddressHoldTime = 0x00;
    fmc_read_handle.DataSetupTime = 0x78;               /* 数据保存时间(DATAST)为78个fmc_ker_ck=4.5*78=351ns */
                                                        /* 因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片 */
    fmc_read_handle.AccessMode = FSMC_ACCESS_MODE_A;     /* 模式A */
    /*FMC写时序控制寄存器*/
    fmc_write_handle.AddressSetupTime = 0x15;           /* 地址建立时间(ADDSET)为15个fmc_ker_ck=67.5ns */
    fmc_write_handle.AddressHoldTime = 0x00;
    fmc_write_handle.DataSetupTime = 0x15;              /* 数据保存时间(DATAST)为15个fmc_ker_ck=67.5ns */
                                                        /* 15个fmc_ker_ck（fmc_ker_ck=220Mhz）,某些液晶驱动IC的写信号脉宽，最少也得50ns。 */
    fmc_write_handle.AccessMode = FSMC_ACCESS_MODE_A;    /* 模式A */
    ...
    if (lcddev.id == 0X9341 || lcddev.id == 0X1963)  /* 如果是这几个IC,则设置WR时序为最快 */
    {
        ...
        FSMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended, &fmc_write_handle, g_sram_handle.Init.NSBank, g_sram_handle.Init.ExtendedMode);
    }
