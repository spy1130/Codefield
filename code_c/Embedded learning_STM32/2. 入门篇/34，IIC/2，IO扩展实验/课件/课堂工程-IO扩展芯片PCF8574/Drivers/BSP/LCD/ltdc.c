...
{ 
    ...
    {
        *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x) + y)) = color; 
    }
#else
    if (lcdltdc.dir)   /* 横屏 */
    {
...
{ 
    ...
    {
        return *(uint32_t *)((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] + lcdltdc.pixsize * (lcdltdc.pwidth * (lcdltdc.pheight - x) + y)); 
    }
#else
    if (lcdltdc.dir)   /* 横屏 */
    {
...
//    addr =((uint32_t)g_ltdc_framebuf[lcdltdc.activelayer] + lcdltdc.pixsize * (lcdltdc.pwidth * psy + psx));
//
//    if (LTDC_PIXFORMAT == LCD_PIXEL_FORMAT_RGB565)  /* 如果是RGB565格式的话需要进行颜色转换，将16bit转换为32bit的 */
//    {
 ...
 * @param       此函数仅支持uint16_t,RGB565格式的颜色数组填充.
 * @param       (sx,sy),(ex,ey)   : 填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)
