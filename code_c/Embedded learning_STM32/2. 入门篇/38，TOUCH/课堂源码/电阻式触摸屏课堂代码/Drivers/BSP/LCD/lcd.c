...
void lcd_init(void)
{
    ...
    /*FSMC读时序控制寄存器*/
    fsmc_read_handle.AddressSetupTime = 0x15;            /* 地址建立时间(ADDSET)为15个fmc_ker_ck 1/220M=4.5ns*15=67.5ns */
    fsmc_read_handle.AddressHoldTime = 0x00;
    fsmc_read_handle.DataSetupTime = 0x78;               /* 数据保存时间(DATAST)为78个fmc_ker_ck=4.5*78=351ns */
                                                        /* 因为液晶驱动IC的读数据的时候，速度不能太快,尤其是个别奇葩芯片 */
    fsmc_read_handle.AccessMode = FSMC_ACCESS_MODE_A;     /* 模式A */
    /*FSMC写时序控制寄存器*/
    fsmc_write_handle.AddressSetupTime = 0x15;           /* 地址建立时间(ADDSET)为15个fmc_ker_ck=67.5ns */
    fsmc_write_handle.AddressHoldTime = 0x00;
    fsmc_write_handle.DataSetupTime = 0x15;              /* 数据保存时间(DATAST)为15个fmc_ker_ck=67.5ns */
