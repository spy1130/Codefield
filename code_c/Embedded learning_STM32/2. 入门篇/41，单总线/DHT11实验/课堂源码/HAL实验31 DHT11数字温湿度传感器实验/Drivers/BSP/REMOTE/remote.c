...
TIM_HandleTypeDef g_tim4_handle;      /* 定时器1句柄 */
...
void remote_init(void)
{
    ...
    g_tim4_handle.Instance = REMOTE_IN_TIMX;                    /* 通用定时器1 */
    ...
    HAL_TIM_IC_Start_IT(&g_tim4_handle, REMOTE_IN_TIMX_CHY);    /* 开始捕获TIM1的通道1 */
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == REMOTE_IN_TIMX)
    {
        ...
        else           /* 下降沿捕获 */
        {
            ...
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim4_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_RISING);/* 配置TIM5通道1上升沿捕获 */
            ...
            if (g_remote_sta & 0X10)        /* 完成一次高电平捕获 */
            {
                if (g_remote_sta & 0X80)    /* 接收到了引导码 */
                {
                    ...
                    if (dval > 300 && dval < 800)   /* 560为标准值,560us */
                    {
                        ...
                        g_remote_data |= 0; /* 接收到0 */
                    }
                    else if (dval > 1400 && dval < 1800)    /* 1680为标准值,1680us */
                    {
                        g_remote_data <<= 1;/* 左移一位. */
                        g_remote_data |= 1; /* 接收到1 */
                    }
...
uint8_t remote_scan(void)
{
    ...
    {
        t1 = g_remote_data >> 24;   /* 得到地址码 */
        t2 = (g_remote_data >> 16) & 0xff;  /* 得到地址反码 */
