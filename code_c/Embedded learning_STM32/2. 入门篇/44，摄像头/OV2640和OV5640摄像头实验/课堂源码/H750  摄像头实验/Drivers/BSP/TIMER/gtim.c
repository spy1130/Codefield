...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == GTIM_TIMX_PWM)
    {
        ...
        gpio_init_struct.Alternate = GTIM_TIMX_PWM_CHY_GPIO_AF; /* 定时器x通道y的GPIO口复用 */
...
void gtim_timx_npwm_chy_init(uint16_t arr,uint16_t psc)
{
    ...
    g_timx_npwm_chy_handle.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_ENABLE; /* 使能TIMx_ARR 寄存器进行缓冲 */
    ...
    HAL_TIM_PWM_ConfigChannel(&g_timx_npwm_chy_handle, &timx_oc_npwm_chy, GTIM_TIMX_NPWM_CHY);/* 配置TIMx通道y */
...
void GTIM_TIMX_NPWM_IRQHandler(void)
{
    ...
    if(__HAL_TIM_GET_FLAG(&g_timx_npwm_chy_handle, TIM_FLAG_UPDATE) != RESET)
    {
        ...
        { 
            ...
            HAL_TIM_GenerateEvent(&g_timx_npwm_chy_handle, TIM_EVENTSOURCE_UPDATE); /* 产生一次更新事件,以更新RCR寄存器 */
            __HAL_TIM_ENABLE(&g_timx_npwm_chy_handle);                              /* 使能定时器TIMX */
        }
        else
        { 
            GTIM_TIMX_NPWM->CR1 &= ~(1 << 0);           /* 关闭定时器TIMX,使用HAL关闭会清除PWM通道信息，所以不能用 */
        }
