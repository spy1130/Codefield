...
void dcmi_init(void)
{
    ...
    HAL_DCMI_DeInit(&g_dcmi_handle);                            /* 先清除以前的设置 */
...
{ 
    ...
    g_dma_dcmi_handle.Init.FIFOMode = DMA_FIFOMODE_DISABLE;             /* 禁止FIFO */
    g_dma_dcmi_handle.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;     /* 禁止FIFO该位不起作用 用于设置阈值 */
    ...
    /* 在开启DMA之前先使用__HAL_UNLOCK()解锁一次DMA,因为HAL_DMA_Start()HAL_DMAEx_MultiBufferStart()
