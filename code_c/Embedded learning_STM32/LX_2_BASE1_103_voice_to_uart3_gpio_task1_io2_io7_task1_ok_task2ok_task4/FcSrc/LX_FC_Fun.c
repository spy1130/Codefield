...
u8 target_altituded(u32 angle_deg)
{
	...
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[4] = BYTE2(angle_deg);
		dt.cmd_send.CMD[5] = BYTE3(angle_deg);
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
	else
...
u8 target_location(u32 angle_deg1,u32 angle_deg2)
{
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CMD[2] = BYTE0(angle_deg1);
		dt.cmd_send.CMD[3] = BYTE1(angle_deg1);
		dt.cmd_send.CMD[4] = BYTE2(angle_deg1);
		dt.cmd_send.CMD[5] = BYTE3(angle_deg1);
		dt.cmd_send.CMD[6] = BYTE0(angle_deg2);
		dt.cmd_send.CMD[7] = BYTE1(angle_deg2);
		dt.cmd_send.CMD[8] = BYTE2(angle_deg2);
		dt.cmd_send.CMD[9] = BYTE3(angle_deg2);
		CMD_Send(0xff, &dt.cmd_send);
		return 1;
	}
	else
...
//旋转(方向:左——left，右——right，角度deg:0-359，速度deg/s:5-90)
u8 Rotate(u8 direction,u32 angle_deg, u32 velocity_degs)
{
	...
	if (dt.wait_ck == 0) //没有其他等待校验的CMD时才发送本CMD
	{
		...
		dt.cmd_send.CID = 0X10;
		dt.cmd_send.CMD[0] = 0X02;
		dt.cmd_send.CMD[1] = direction;    //这里是旋转方向，头文件进行了宏定义，left(0x07),right(0x08)
		//
		dt.cmd_send.CMD[2] = BYTE0(angle_deg);
		dt.cmd_send.CMD[3] = BYTE1(angle_deg);
