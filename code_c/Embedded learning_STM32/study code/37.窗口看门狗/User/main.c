...
#include "OLED.h"
#include "Key.h"
//对喂狗时间要求高，在特定时间段喂狗，不独立工作，时钟来自PCLK1（RCC）
int main(void)
{
	/*模块初始化*/
	OLED_Init();						//OLED初始化
	...
	/*开启时钟*/
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_WWDG, ENABLE);	//开启WWDG的时钟（APB1）
	
	/*WWDG初始化*/
	WWDG_SetPrescaler(WWDG_Prescaler_8);			//设置预分频为8
	WWDG_SetWindowValue(0x40 | 21);					//设置窗口值，窗口时间为30ms（0X40用于使第六位寄存器为1）
