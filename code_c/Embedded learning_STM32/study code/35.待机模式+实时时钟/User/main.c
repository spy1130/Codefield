...
int main(void)
{
	...
	/*开启时钟*/
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);	//开启PWR的时钟，虽然在MyRTC里已经开过，但是再开一遍，防止bug，降低程序之间的耦合性
														//停止模式和待机模式一定要记得开启
	...
	while (1)
	{
		...
		PWR_EnterSTANDBYMode();						//STM32进入待机模式，并等待指定的唤醒事件（WKUP上升沿或RTC闹钟）
		/*待机模式唤醒后，程序会重头开始运行*/
	}
}
